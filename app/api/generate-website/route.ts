...
const RequestSchema = z.object({
  ...
  pages: z.array(z.string()).optional().default(['Home', 'About', 'Pricing', 'Contact', 'Dashboard']),
...
export async function POST(req: Request) {
  ...
  const { prompt, stylePreset, pages, projectId } = parsed.data;
...
Generate these specific pages: ${pageList}
For each page, include 3-5 appropriate sections with realistic placeholder content.
Include a navigation with links to all pages and a footer with copyright.
Use unique short IDs for id fields (e.g. "home-hero", "about-team").
Set isHome: true for the first/home page.
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user', content: `Build a full multi-page website for: ${prompt}` },
      ],
    ...
    if (!uiOutput.success) {
      return NextResponse.json(
        { error: 'AI response did not match expected schema', details: uiOutput.error.flatten() },
        { status: 500 }
      );
    }

    // Save version if projectId provided and authenticated
