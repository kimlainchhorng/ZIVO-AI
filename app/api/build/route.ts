...
async function runWebsiteV2Pipeline(
  ...
  // Inject placeholder SVG assets (icons, illustrations, og, favicon) for any
  // path that wasn't already generated by the AI.
  const existingAssetPaths = new Set(files.map((f) => f.path));
  const placeholderAssets = getAllPlaceholderAssets(plan.brand.name, plan.brand.tagline);
