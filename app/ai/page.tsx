...
const WebsiteBuilderPanel = dynamic(() => import("./WebsiteBuilderPanel"), { ssr: false });
const MobileBuilderPanel = dynamic(() => import("./MobileBuilderPanel"), { ssr: false });
import { CreatePageModal, GitHubPushModal } from "./BuilderModals";
const CodeBuilderLeftPanel = dynamic(() => import("./CodeBuilderLeftPanel"), { ssr: false });
const CodeBuilderRightPanel = dynamic(() => import("./CodeBuilderRightPanel"), { ssr: false });
import TopNavBar from "./TopNavBar";
import MediaGenerationPanels from "./MediaGenerationPanels";
import LeftPanelBottomActions from "./LeftPanelBottomActions";
import CollapsibleBuildOutput from "./CollapsibleBuildOutput";
import ContinueBuildBar from "./ContinueBuildBar";
    ...
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: COLORS.bg, color: COLORS.textPrimary, fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif", overflow: "hidden" }}>
        ...
        <TopNavBar
          ...
          setSupabaseToken={setSupabaseToken}
          setSupabaseUserEmail={setSupabaseUserEmail}
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          ...
          <div style={{ width: leftPanelCollapsed ? 0 : `${leftPanelWidth}px`, flexShrink: 0, display: "flex", flexDirection: "column", borderRight: `1px solid ${COLORS.border}`, background: COLORS.bgPanel, overflow: "hidden", transition: leftPanelCollapsed ? "width 0.2s ease" : "none", position: "relative" }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "1.25rem" }}>
              ...
              {/* ── Code Builder Mode ── */}
              {/* @ts-expect-error - Component props mismatch during refactor */}
              {mode === "code" && <CodeBuilderLeftPanel
                ...
                hasFiles={hasFiles}
                output={output}
                setActiveRightTab={setActiveRightTab as (v: string) => void}
              ...
              {/* ── Image/Video/3D Modes ── */}
              <MediaGenerationPanels
                mode={mode}
                ...
                setVideoFrameCount={setVideoFrameCount}
                videoError=""
                ...
                videoZipError=""
                threeDPrompt={threeDPrompt}
                setThreeDPrompt={setThreeDPrompt}
                threeDSceneType={threeDSceneType as "object" | "scene" | "character"}
                setThreeDSceneType={setThreeDSceneType as (v: "object" | "scene" | "character") => void}
                threeDLoading={threeDLoading}
                handle3DGenerate={handle3DGenerate}
                threeDResult={threeDResult?.html ?? null}
                threeDError={threeDError ?? ""}
                threeDShowSource={threeDShowSource}
                setThreeDShowSource={setThreeDShowSource}
              />
            ...
            <LeftPanelBottomActions
              mode={mode}
              hasFiles={hasFiles}
              showVercelTokenInput={showVercelTokenInput}
